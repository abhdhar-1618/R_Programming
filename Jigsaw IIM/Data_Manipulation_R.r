...
setwd("/Users/bharadwajaldur/Desktop/Learning/AI/Data Manipulation")
...
#Use the iris dataset in R.It is an inbuilt dataset.
#1.Study the dataset.
#2.Create Petal.Length to Petal.Width ratio
data(iris) #load the data into workspace
head(iris)
dim(iris)
library(help = "datasets") #information about any package. datasets is a default package which gets downloaded-
#when base R gets downloaded and therefore we need not separately install this package
...
retail[2,]
retail$Cost
#order a column in a table
order(retail$Cost,decreasing=TRUE)
retail$Cost[order(retail$Cost,decreasing=TRUE)]
head(retail)
...
#skip this (increasing and decreasing) - explain arrange in dplyr to do this easily
order(as.character(retail$Item_Category))
#we observe that repeated ones are given different numbers
sorted_1 <- retail[order(order(as.character(retail$Item_Category),decreasing=TRUE),retail$Revenue,decreasing=TRUE),]
head(sorted_1)
#sorting is wrong  
rank(as.character(retail$Item_Category)) #gives repeated values same number
x<-c(1,1,2,3,1)
rank(x)
order(x)
sorted_2 <- retail[order(-rank(as.character(retail$Item_Category)),retail$Revenue,decreasing=TRUE),]
head(sorted_2)                      
#how does it work for sorted<-retail[order(retail$Item_Category,retail$Revenue,decreasing=TRUE),] then?
#This is because both items are inside order and then order takes care of it
